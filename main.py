...
app = FastAPI(title="Hệ thống Tự động hóa Hợp đồng Vô Đối")
...
# =====================================================================
# CÁC HÀM XỬ LÝ DỮ LIỆU CHUNG & LÕI DOCX
# =====================================================================
...
@app.get("/")
def read_root():
    return {"message": "Hệ thống Backend Vô Đối đang chạy mượt mà trên Render!"}


# =====================================================================
# API 1: HỢP ĐỒNG GẠO (PREFIX 1)
# =====================================================================
def format_number_gao(val):
    try:
        if pd.isna(val) or val == "" or str(val).lower() == 'nan': return ""
        return f"{float(val):,.0f}".replace(',', '.')
    except ValueError: return str(val)

def format_cell_gao(cell, text, bold=False, align='center'):
    cell.text = str(text)
...
def autofit_to_window_gao(table):
    tblPr = table._tbl.tblPr
...
async def tao_hop_dong_gao(excel_file: UploadFile = File(...), word_template: UploadFile = File(...)):
    ...
    sohd_dau = str(danh_sach_sohd[0]).zfill(2) if danh_sach_sohd else "00"
    sohd_cuoi = str(danh_sach_sohd[-1]).zfill(2) if danh_sach_sohd else "00"
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for sohd, group in nhom_hop_dong:
            ...
            ve_vien_ngang_gao(bang_1)
            autofit_to_window_gao(bang_1)
            
            hdr1 = bang_1.rows[0].cells
            format_cell_gao(hdr1[0], 'Tên hàng', bold=True, align='center')
            format_cell_gao(hdr1[1], 'Số lượng', bold=True, align='center')
            format_cell_gao(hdr1[2], 'Đơn giá', bold=True, align='center')
            format_cell_gao(hdr1[3], 'Mặt hàng', bold=True, align='center')
            
            for i, (_, row) in enumerate(group.iterrows()):
                r1 = bang_1.rows[i + 1].cells
                format_cell_gao(r1[0], str(row['tenhang']) if pd.notna(row['tenhang']) else "", align='center')
                format_cell_gao(r1[1], f"{format_number_gao(row['soluong'])} kg", align='center')
                format_cell_gao(r1[2], f"{format_number_gao(row['dongia'])} đ/kg", align='right') 
                format_cell_gao(r1[3], str(row['mathang']) if pd.notna(row['mathang']) else "", align='center')

            bang_subdoc = doc.new_subdoc()
            bang = bang_subdoc.add_table(rows=so_mat_hang + 3, cols=5)
            bang.style = 'Table Grid'
            autofit_to_window_gao(bang) 
            
            hdr_cells = bang.rows[0].cells
            format_cell_gao(hdr_cells[0], 'STT', bold=True, align='center')
            format_cell_gao(hdr_cells[1], 'Tên hàng hóa', bold=True, align='center')
            format_cell_gao(hdr_cells[2], 'Số lượng (Kg)', bold=True, align='center')
            format_cell_gao(hdr_cells[3], 'Đơn Giá', bold=True, align='center')
            format_cell_gao(hdr_cells[4], 'Thành Tiền', bold=True, align='center')
            
            for i, (_, row) in enumerate(group.iterrows()):
                row_cells = bang.rows[i + 1].cells
                format_cell_gao(row_cells[0], str(i + 1), align='center')
                format_cell_gao(row_cells[1], str(row['tenhang']) if pd.notna(row['tenhang']) else "", align='center')
                format_cell_gao(row_cells[2], format_number_gao(row['soluong']), align='center')
                format_cell_gao(row_cells[3], format_number_gao(row['dongia']), align='right') 
                format_cell_gao(row_cells[4], format_number_gao(row['thanhtien']), align='right') 
            ...
            format_cell_gao(thue_row[0], thue_txt, bold=True, align='center')
            format_cell_gao(thue_row[4], format_number_gao(tong_thue_hop_dong), bold=True, align='right') 
            
            tong_row = bang.rows[so_mat_hang + 2].cells
            tong_row[0].merge(tong_row[3])
            format_cell_gao(tong_row[0], "Tổng tiền thanh toán", bold=True, align='center')
            format_cell_gao(tong_row[4], format_number_gao(tong_tien_hop_dong), bold=True, align='right') 
        ...
        if danh_sach_file:
            master_file_name = f"1_MASTER_HOP_DONG_TONG_{sohd_dau}_Den_{sohd_cuoi}.docx"
            merge_docs(danh_sach_file, os.path.join(temp_dir, master_file_name))
...
# =====================================================================
def format_number_xetai(val):
    try:
        if pd.isna(val) or val == "" or str(val).lower() == 'nan': return ""
        return str(int(float(val)))
    except ValueError: return str(val)

def format_money_xetai(val):
    try:
        if pd.isna(val) or val == "" or str(val).lower() == 'nan': return ""
        return f"{float(val):,.0f}".replace(',', '.')
    except ValueError: return str(val)

def format_cell_xetai(cell, text, bold=False, align='center'):
    cell.text = str(text)
    cell.vertical_alignment = WD_CELL_VERTICAL_ALIGNMENT.CENTER 
    p = cell.paragraphs[0]
    p.paragraph_format.space_before = Pt(6)
...
def autofit_to_window_xetai(table):
    tblPr = table._tbl.tblPr
    tblW = tblPr.find(qn('w:tblW'))
    if tblW is None:
        tblW = OxmlElement('w:tblW')
...
async def tao_hop_dong_xe_tai(excel_file: UploadFile = File(...), word_template: UploadFile = File(...)):
    ...
    if max_score < 3: 
        return {"error": "Không tìm thấy dòng tiêu đề trong file excel."}
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for index, row in df.iterrows():
            ...
            trongtai = format_number_xetai(lay_gia_tri(row, ['trongtai', 'trọng tải', 'tải']))
            
            tien_thang = format_money_xetai(lay_gia_tri(row, ['tien', 'tiền', 'giá']))
            so_thang = format_number_xetai(lay_gia_tri(row, ['thang', 'tháng']))
            tong_tien = format_money_xetai(lay_gia_tri(row, ['tong', 'tổng', 'thành']))

            bang_subdoc = doc.new_subdoc()
            bang = bang_subdoc.add_table(rows=3, cols=5)
            ke_vien_full_xetai(bang) 
            autofit_to_window_xetai(bang)
            
            hdr = bang.rows[0].cells
            format_cell_xetai(hdr[0], 'Từ tháng', bold=True, align='center')
            format_cell_xetai(hdr[1], 'Đến tháng', bold=True, align='center')
            format_cell_xetai(hdr[2], 'Số tháng', bold=True, align='center')
            format_cell_xetai(hdr[3], 'Giá trị thuê / tháng', bold=True, align='center')
            format_cell_xetai(hdr[4], 'Thành tiền', bold=True, align='center')
            
            r1 = bang.rows[1].cells
            format_cell_xetai(r1[0], '01', align='center') 
            format_cell_xetai(r1[1], '12', align='center') 
            format_cell_xetai(r1[2], so_thang, align='center')
            format_cell_xetai(r1[3], tien_thang, align='right') 
            format_cell_xetai(r1[4], tong_tien, align='right')   
            
            r2 = bang.rows[2].cells
            r2[0].merge(r2[3]) 
            format_cell_xetai(r2[0], 'Tổng cộng:', bold=True, align='center')
            format_cell_xetai(r2[4], tong_tien, bold=True, align='right')
        ...
        if danh_sach_file:
            merge_docs(danh_sach_file, os.path.join(temp_dir, '2_00_Ban_In_Tat_Ca_Hop_Dong.docx'))
...
# =====================================================================
def format_currency_api3(value):
    try:
...
async def tao_hop_dong_nguyen_tac(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        hdnt_folder = os.path.join(temp_dir, 'ChiTiet_HopDongNguyenTac')
        bbgn_folder = os.path.join(temp_dir, 'ChiTiet_BienBanGiaoNhan')
        ...
        for sohd, group in grouped_by_invoice:
            ...
            for idx, row in group.reset_index().iterrows():
                items.append({
                    ...
                    'so_luong': format_currency_api3(row['soluong']),
                    'don_gia': format_currency_api3(row['dongia']),
                    'thanh_tien': format_currency_api3(row['thanhtien'])
                })
            ...
            if len(group) == 1:
                ...
                tong_thue = group['thue'].sum(skipna=True)
                tong_tien = group['tongtien'].sum(skipna=True)
            ...
            for p in doc_bbgn.docx.paragraphs:
                if '[CHEN_BANG_VAO_DAY]' in p.text:
                    ...
                    style_cell_api3(tax_row[4], format_currency_api3(tong_thue), WD_ALIGN_PARAGRAPH.RIGHT)
                    
                    total_row = table.add_row().cells
                    total_row[0].merge(total_row[3])
                    style_cell_api3(total_row[0], "Tổng tiền thanh toán", WD_ALIGN_PARAGRAPH.CENTER)
                    style_cell_api3(total_row[4], format_currency_api3(tong_tien), WD_ALIGN_PARAGRAPH.RIGHT)
